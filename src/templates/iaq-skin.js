...
(function () {
  ...
  function applyTheme(th) {
    ...
    if (p) {
      ...
      /* حبرُ الأيقونة أبيضُ لأنّ خلفيّتَها أعلاه تدرّجٌ غامقٌ من لون الأساس.
         كان rgb2hex(p6) — أي الغامقَ نفسَه على الغامق، تضادًّا ١:١، فأيقوناتُ
         «من نحن» تختفي تمامًا. والمبنيّ سليمٌ (--icon-ink:#fff مع التدرّج في
         الكتلة الأولى، و#0c6c6c مع خلفيّةٍ فاتحة في الثانية) — فالعطبُ كان
         يظهر لحظةَ حفظِ أيّ ثيمٍ لأنّ النمطَ السطريَّ يغلب الكتلتين. */
      set('--icon-ink', '#fff');
    ...
    /* خلفيةُ صدر الصفحة حقلٌ مستقلٌّ في اللوحة، والفارغُ يعني «كما بُني».
       كان يرتدّ إلى لون العمق الغامق، فلم يكن للمدير سبيلٌ من اللوحة إلى
       إعادة الخلفية الفاتحة المبنيّة (#edf6f5) أصلًا: تفريغُ الحقل يُعطيه
       غامقًا آخر. والصمتُ هنا هو الصواب — ما لا يُحدّده المدير يبقى كما بُني. */
    var hb = hex2rgb(th.heroBg);
    if (hb) set('--hero-bg', rgb2hex(hb));
    else root.style.removeProperty('--hero-bg');
  ...
  /* المطابقة بـdata-soc لا بالتسمية العربية: لا تتعلّق بلغة الواجهة */
  var SOC = { social_x: 'x', social_youtube: 'youtube', social_linkedin: 'linkedin',
  ...
  function applyContact() {
    ...
    for (var fk in FIELDS) {
      ...
      var fv = IAQ.settings()[fk];
      if (fv === undefined || fv === null) continue;
      var txt = String(fv).trim();
      if (!txt) continue;                          /* فارغ = يبقى المبنيّ */
      [].slice.call(document.querySelectorAll('[data-iaq-f="' + FIELDS[fk] + '"]'))
        .forEach(function (el) { el.textContent = txt; n++; });
    }
  ...
  /* ------------------- خلفيّتا الترويسة والتذييل -------------------
     أنماطٌ يختارها المدير. والقيمة تُركَّب هنا لا في CSS، فيبقى في الصفحة
     متغيّرٌ واحدٌ لكل سطحٍ ولا يتغيّر ترتيب الطبقات. */
  function css_url(u) {
    /* الرابط بين قوسين مزدوجَي الاقتباس، وما فيه من اقتباسٍ يُهرَّب: رابطٌ
       فيه " يكسر القيمة ويُبطل الخلفية كلَّها. */
    return 'url("' + String(u).replace(/["\\]/g, '\\$&') + '")';
  }
  function pct(v, dflt) {
    var n = Number(v);
    return (isFinite(n) && n >= 0 && n <= 100) ? n : dflt;
  }
  function flat(hex, alpha) {
    var c = hex2rgb(hex);
    return c ? rgba(c, String(alpha)) : 'rgba(0,0,0,' + alpha + ')';
  }

  /* يبني قيمة background من نمطٍ ومعاملاته. يُرجع '' إن لم يكتمل المطلوب —
     فيبقى المبنيّ كما هو ولا نضع خلفيةً نصفَ مضبوطة. */
  function surface(mode, c1, c2, angle, img, overlay, fallback) {
    var a = Number(angle);
    if (!isFinite(a)) a = 135;
    if (mode === 'image') {
      if (!img) return '';
      var ov = pct(overlay, 55) / 100;
      var tint = flat(c1 || fallback, ov.toFixed(2));
      /* التدرّج المسطّح فوق الصورة يُقرأ نصًّا في الطبقة نفسها */
      return 'linear-gradient(' + tint + ',' + tint + '), ' + css_url(img) + ' center/cover no-repeat';
    }
    if (mode === 'gradient') {
      if (!c1 || !c2) return '';
    ...
    return '';                       /* 'glass' أو غير معروف: يبقى المبنيّ */
  }

  function applyChrome() {
    var n = 0;
    /* يُزيل عند الفراغ لا يتجاهل: بلا الإزالة يبقى ما كُتب سابقًا فلا يستطيع
       المدير العودةَ إلى المبنيّ بعد أن جرّب تدرّجًا. */
    function put(k, v) {
      if (v) { root.style.setProperty(k, v); n++; }
      else if (root.style.getPropertyValue(k)) { root.style.removeProperty(k); n++; }
    }
    /* للمتغيّرات التي يملك المظهرُ قيمتها الاحتياطيّة: تُكتب ولا تُزال */
    function keep(k, v) { if (v) { root.style.setProperty(k, v); n++; } }

    /* ---- الترويسة ---- */
    var hm = String(IAQ.setting('header_mode') || '').trim();
    var hbg = surface(hm, IAQ.setting('header_color'), IAQ.setting('header_color2'),
                      IAQ.setting('header_angle'), IAQ.setting('header_image'),
                      IAQ.setting('header_overlay'), '#ffffff');
    put('--header-bg', hbg);
    /* الحالة الثانية (بعد التمرير) نفس السطح: وإلّا وقع وميضٌ عند التمرير */
    put('--header-bg2', hbg);
    var hi = IAQ.setting('header_ink');
    put('--nav-ink', hi);
    put('--brand-en', hi);
    keep('--nav-hover', IAQ.setting('header_hover'));   /* المظهر يملك الاحتياط */
    put('--header-line', IAQ.setting('header_line'));

    /* ---- التذييل ---- */
    var fm = String(IAQ.setting('footer_mode') || '').trim();
    var fbg = surface(fm, IAQ.setting('footer_color'), IAQ.setting('footer_color2'),
                      IAQ.setting('footer_angle'), IAQ.setting('footer_image'),
                      IAQ.setting('footer_overlay'), '#04342c');
    put('--foot-bg', fbg);
    put('--foot-ink', IAQ.setting('footer_ink'));
    put('--foot-head', IAQ.setting('footer_head'));
    put('--foot-line', IAQ.setting('footer_line'));
    return n;
  }
  ...
  function applyAll() {
    ...
    /* الخطُّ متزامنٌ كالألوان — لا في later.
       كان داخل later التي تنتظر DOMContentLoaded، فيقع أوّلُ رسمٍ بالخطّ
       المبنيّ ثمّ يُستبدَل: وميضٌ يراه المدير في كل فتحةٍ للموقع. والدالّةُ
       لا تحتاج شجرةً أصلًا — تضبط متغيّرًا على :root وسمةً عليه، وكلاهما
       متاحٌ لحظةَ تحليل <head>. والألوانُ تُطبَّق متزامنةً سلفًا (applyTheme
       أعلاه)، فلهذا لم يكن للّون وميضٌ وكان للخطّ. */
