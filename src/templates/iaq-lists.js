...
(function () {
  ...
  /* مصدرُ صورةٍ: رابطٌ كامل كما هو، وإلّا فاسمُ ملفٍّ داخل مجلّد البناء.
     دالّةٌ واحدةٌ لكلّ القوائم — كانت ثلاث نسخٍ متفرّقة وواحدةٌ ناقصة. */
  function imgSrc(v, dir) {
    var s = String(v || '');
    if (!s) return '';
    if (/^(https?:)?\/\//.test(s) || s.charAt(0) === '/' || /^data:/.test(s)) return s;
    return dir + s;
  }
    ...
    if (kind === 'assembly') {
      ...
      if (ph) {
        ...
        if (row.photo) {
          ...
          if (img) {
            /* رابطٌ كامل يُقبل كما هو — كالأخبار والشركاء. كان يُلصَق دائمًا
               بعد img/board/ فتنكسر كلُّ صورةٍ تُختار من مكتبة اللوحة. */
            img.setAttribute('src', imgSrc(row.photo, 'img/' + kind + '/'));
  ...
  function logoSrc(logo) { return imgSrc(logo, 'img/partners/'); }
  function partners() {
  ...
  function newsImg(f) { return imgSrc(f, 'img/news/'); }
