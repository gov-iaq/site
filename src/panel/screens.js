...
window.IAQ_SCREENS = (function () {
  ...
  var SCREENS = {
    ...
    heroslides: {
      ...
      reach: 'يسري على الصفحة الرئيسة عند أوّل تحميل. ولا يُفرَّغ صدرُ الصفحة أبدًا: ' +
    ...
    footcfg: {
      ...
      rows: [
        ...
        { key: 'footer_overlay', l: 'تعتيم فوق الصورة %', t: 'int', def: 78, min: 0, max: 100, half: 1,
          hint: 'التذييل نصُّه فاتحٌ، فيحتاج تعتيمًا أقوى من الشريط العلويّ' },
    ...
    contactcfg: {
      ...
      kind: 'settings',
      reach: 'يسري على الشريط العلويّ والتذييل وصفحة «تواصل معنا» في كل الصفحات عند أوّل تحميل. ' +
    ...
    sitecfg: {
      ...
      rows: [
        ...
        { key: 'site_logo', l: 'شعار الموقع', t: 'img', def: '',
          ...
          hint: 'اختر صورة فتُرفع ويُستبدل الشعار في الشريط العلويّ والتذييل وخلفية صدر الصفحة. ' +
        ...
        { key: 'site_font', l: 'خطّ المنصّة', t: 'select', def: 'IBM Plex Sans Arabic',
          ...
          hint: 'يسري على كل نصوص الموقع: العناوين والنصوص واسم الجمعية معًا. ' +
                ...
                'وفراغات القائمة تُشدّ آليًّا مع الخطوط العريضة كي لا يفيض الشريط العلويّ.' },
      ]
    },
    subslist: {
    ...
    Promise.all([
      ...
      if (nbox) {
        ...
        Promise.all([
          ...
          if (!alive(myKey)) return;
          var more = [];
          /* countOf تُرجع -1 عند فشل القراءة (screens.js:1370). ولو تخطّيناه
             بـ«> 0» لَظهر «لا شيءَ ينتظرك» وفي الحقيقة لم نفحص — وهو الكذبُ
             نفسه الذي أُصلح في مؤشّرات الوارد. */
          if (c[0] < 0 || c[1] < 0) {
            more.push({ urgent: 1, icon: 'alert', view: 'dashboard',
                        txt: 'تعذّر فحصُ المسودّات المعلّقة',
                        sub: 'لم تُقرأ الأخبارُ أو الوثائق — قد تكون هناك مسودّاتٌ ' +
                             'لا يراها الزائر. حدِّث الصفحة أو تحقّق من اتصالك' });
          }
          if (c[0] > 0) {
          ...
          if (nb2) {
            ...
            nb2.hidden = !!any;
            /* «لا شيء» تُقال حين فُحص كلُّ شيءٍ فعلًا — لا حين تعذّر الفحص */
    ...
    Promise.all([
      ...
      var html = healthNotice(r[6]) +
        card('الخلاصة',
          ...
          'بالضرورة، ' +
          /* الوعدُ بالسقف لا يُقال إلّا إن كان السقفُ قائمًا فعلًا: منظرُ الصحّة
             لا يوجد قبل ترقية v11، فقولُ «سقفٌ يحدُّ التضخيم» حينها كذب. */
          (r[6]
            ? 'وسقفٌ يوميّ (' + Number(r[6].cap) + ' سطرًا) يحدُّ أيَّ تضخيم، ' +
              'وأيُّ يومٍ يبلغه يُعلَن أعلاه.'
            : 'ولا سقفَ للتسجيل بعد — شغّل supabase/schema-v11.sql ليصير للأرقام ' +
              'حدٌّ يمنع تلفيقَها ويُعلَن بلوغُه.')) +
